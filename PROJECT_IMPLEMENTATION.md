# Subtitle Generator — Standalone Real GUI Implementation
...
Default URL: `http://127.0.0.1:9156`

## What is inside this project folder

- `app/` — FastAPI backend for this project.
- `static/` — elegant browser GUI.
- `plugins/subtitle-generator.json` — this project’s own feature/customization/input schema.
...
- `source_target_language` — source/target language (select)
- `max_chars_line` — max chars/line (text)
- `cps_wpm` — CPS/WPM (text)
- `speaker_labels` — speaker labels (text)
- `glossary` — glossary (text)
- `subtitle_format` — subtitle format (select)
