...
**Domain:** `Media / Subtitles` · **Suite:** `Media Creator Suite` · **Accent:** `#fb7185`
...
**Subtitle Generator** is a standalone, browser-based AI workflow app for **Media / Subtitles**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Media → subtitles with speakers/translations.
...
|---|---:|:---:|---|---|
| `media`<br>Media | text | Yes | — | Affects input: Media. |
...
| `source_target_language`<br>source/target language | select | No | English | Affects customization: source/target language. |
| `max_chars_line`<br>max chars/line | text | No | — | Affects customization: max chars/line. |
| `cps_wpm`<br>CPS/WPM | text | No | — | Affects customization: CPS/WPM. |
| `speaker_labels`<br>speaker labels | text | No | — | Affects customization: speaker labels. |
| `glossary`<br>glossary | text | No | — | Affects customization: glossary. |
| `subtitle_format`<br>subtitle format | select | No | — | Affects customization: subtitle format. |
...
- **source/target language**: English, Arabic, Egyptian Arabic, French, German, Spanish
...
uvicorn app.main:app --host 127.0.0.1 --port 9156
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9156`. |
...
- Media
- subtitles with speakers/translations
...
- Export download
- Job history
- STT provider required for raw audio
...
```bash
curl http://127.0.0.1:9156/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9156/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9156 uvicorn app.main:app --host 0.0.0.0 --port 9156
```
...
- `plugins/subtitle-generator.json` for plugin metadata.
